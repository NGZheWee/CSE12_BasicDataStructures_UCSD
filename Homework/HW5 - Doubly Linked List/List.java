...
public class List extends Base {
	...
	private class ListEngine {
		...
		*/
		 ...
		 * @param element Base: Values of element are expected to be a Base
		...
		*/
		 ...
		 * @return Base: the data of the removed node.
		*/
		Base remove(long where) {
			...
			if (location != null) {
				...
				Base result = location.remove();// retrieve the result firstly
				if (where == 0) {
					advancePre();
				}
		 ...
		 * @return Base: the data stored at location where.
		 */
		...
		private class Node {
			...
			private class NodeEngine {
				 ...
				 * 						belongs to.
				 * @param element Base: Values of element are expected to be a
				 * 						Base object we wish to store in the new
				 * 						node.
				...
				*/
				 ...
				 * 						 inserted AFTER.
				 * @param element Base: Values of element are expected to be a
				 *                      Base object we wish to store in the
				 * 						new node
				...
				*/
				 ...
				 * @return Base: A reference to the data is returned.
				 */
				Base remove() {
					Base storeBase = data;// store the data before jettison.
